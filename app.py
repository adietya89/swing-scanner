...
with col2:
    st.markdown("""
    <h1 style="color: white; font-weight: 800; margin-bottom: 4px;">📈 Swing Trading Scanner</h1>
...
for t in TICKERS:
    try:
        ...
        zone = detect_zone(df_t)
        candle,bias = detect_candle(df_t)
...
df = df.sort_values(by=["Confidence","Signal","RSI"],ascending=[False,False,True])
...
h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11,h12 = st.columns([1.2,1,1,1,1,0.8,1.2,1.2,1,1,1,1])
h1.markdown("**Kode**"); h2.markdown("**Harga**"); h3.markdown("**Signal**"); h4.markdown("**Trend**")
h5.markdown("**Zone**"); h6.markdown("**Candle**"); h7.markdown("**MA >**"); h8.markdown("**MACD**")
h9.markdown("**RSI**"); h10.markdown("**TP**"); h11.markdown("**SL**"); h12.markdown("**SPARKLINE**")
...
for _, row in df.iterrows():
    ...
    with c1.container(height=ROW_HEIGHT): st.write(row["Kode"].replace(".JK",""))
    with c2.container(height=ROW_HEIGHT): st.write(row["Harga"])
    with c3.container(height=ROW_HEIGHT):
        if row["Signal"]=="BUY": st.markdown("<span style='color:#00C176; font-weight:bold'>BUY</span>",unsafe_allow_html=True)
        else: st.markdown("<span style='color:#999'>HOLD</span>",unsafe_allow_html=True)
    with c4.container(height=ROW_HEIGHT):
        color = "#00C176" if row["Trend"]=="Bullish" else "#FF4D4D"
        text = "🟢 Bullish" if row["Trend"]=="Bullish" else "🔴 Bearish"
        st.markdown(f"<span style='font-size:13px; color:{color}; font-weight:600'>{text}</span>",unsafe_allow_html=True)
    with c5.container(height=ROW_HEIGHT):
        if row["Zone"]=="BUY ZONE": st.markdown("<span style='color:#00C176; font-weight:600'>BUY</span>",unsafe_allow_html=True)
        elif row["Zone"]=="SELL ZONE": st.markdown("<span style='color:#FF4D4D; font-weight:600'>SELL</span>",unsafe_allow_html=True)
        else: st.markdown("<span style='color:#999'>MID</span>",unsafe_allow_html=True)
    with c6.container(height=ROW_HEIGHT): st.pyplot(plot_last_2_candles(row["_df"]), clear_figure=True)
    with c7.container(height=ROW_HEIGHT): st.write(row["MA_Pos"])
    with c8.container(height=ROW_HEIGHT):
        macd = row["MACD"]
        if macd=="Golden Cross": st.markdown("🟢 **Golden Cross**")
        elif macd=="Death Cross": st.markdown("🔴 **Death Cross**")
        else: st.markdown("⚪ Normal")
    with c9.container(height=ROW_HEIGHT):
        rsi=row["RSI"]
        if rsi<40: st.markdown(f"🟢 **{rsi}**")
        elif rsi>70: st.markdown(f"🔴 **{rsi}**")
        else: st.markdown(f"⚪ {rsi}")
    with c10.container(height=ROW_HEIGHT): st.markdown(f"<span style='font-size:13px'>💰 {row['TP']}</span>",unsafe_allow_html=True)
    with c11.container(height=ROW_HEIGHT): st.markdown(f"<span style='font-size:13px'>🛑 {row['SL']}</span>",unsafe_allow_html=True)
    with c12:
        try:
            close = row["_df"]["Close"].tail(90)
            vals = close.to_numpy()
            minv,maxv = vals.min(),vals.max()
            norm = np.full_like(vals,0.5) if maxv-minv==0 else (vals-minv)/(maxv-minv)
            norm = np.clip(norm - np.mean(norm)+0.5,0,1)
            color="#999"
            if vals[-1]>vals[0]: color="#00C176"
            elif vals[-1]<vals[0]: color="#FF4D4D"
            data=pd.DataFrame({"index":range(len(norm)),"close":norm})
            chart=alt.Chart(data).mark_line(color=color,strokeWidth=1.8).encode(
...
# =====================
# UI TABLE MOBILE (RINGKAS)
# =====================
st.markdown('<div class="mobile-expander">',unsafe_allow_html=True)
st.subheader("📱 Market Signal (Mobile)")
if df.empty:
    st.warning("Belum ada data")
else:
    for _,row in df.iterrows():
        st.markdown(f"**{row['Kode'].replace('.JK','')}** | Harga: {row['Harga']} | Signal: {row['Signal']}")
st.markdown('</div>',unsafe_allow_html=True)
...
if top_buy.empty:
    ...
    st.dataframe(
        top_buy[["Kode","Harga","Trend","Zone","Candle","RSI","TP","SL","Confidence"]],
